...
users_ary.each do |u|
    delusers << user_id = u.fetch("Id").fetch("Value") 
end

p delusers.size.to_s + 'users will be deleted!'

delusers.each do |du|
  uri_udel = URI.parse('https://as' + cn + '.awmdm.com/API/system/users/' + du.to_s + '/delete')
  http_udel = Net::HTTP.new(uri_udel.host, uri_udel.port)
  http_udel.use_ssl = true

  req_udel = Net::HTTP::Delete.new(uri_udel.request_uri)
